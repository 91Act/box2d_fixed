...
public:
	...
	{		
		b2Body* ground = NULL;
		{
			...
			float const PlatformWidth = 8_fx;
			...
			*/
			float const Angle1Degrees = -30_fx;
			float const Angle2Degrees = 10_fx;
			
			/*
			The larger the value of SlopeLength, the less likely the glitch will show up.
			*/
			float const SlopeLength = 2_fx;

			float const SurfaceFriction = "0.2"_fx;

			// Convert to radians
			float const Slope1Incline = -Angle1Degrees * b2_pi / 180_fx;
			float const Slope2Incline = Slope1Incline - Angle2Degrees * b2_pi / 180_fx;
			//
			...
			// Horizontal platform
			verts.emplace_back(-PlatformWidth, 0_fx);
			verts.emplace_back(0_fx, 0_fx);
...

		{
			...
			float const BodyWidth = 1_fx;
			float const BodyHeight = "2.5"_fx;
			float const SkiLength = 3_fx;

			/*
			Larger values for this seem to alleviate the issue to some extent.
			*/
			float const SkiThickness = "0.3"_fx;

			float const SkiFriction = 0_fx;
			float const SkiRestitution = "0.15"_fx;
			...
			ski_back_shape.m_p.Set(-SkiLength / 2_fx, -BodyHeight / 2 - SkiThickness * (2_fx / 3));
			ski_back_shape.m_radius = SkiThickness / 2;

			b2CircleShape ski_front_shape;
			ski_front_shape.m_p.Set(SkiLength / 2, -BodyHeight / 2 - SkiThickness * (2_fx / 3));
			...
			skier->SetLinearVelocity(b2Vec2("0.5"_fx, 0_fx));

			m_skier = skier;
		}

		g_camera.m_center = b2Vec2(m_platform_width / 2_fx, 0_fx);
		g_camera.m_zoom = "0.4"_fx;
		m_fixed_camera = true;
	}
