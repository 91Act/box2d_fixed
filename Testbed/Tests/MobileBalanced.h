...
public:
	...
	MobileBalanced()
	{
		...
		{
			b2BodyDef bodyDef;
			bodyDef.position.Set(0_fx, 20_fx);
		...
		b2Body* root = AddNode(ground, b2Vec2_zero, 0, 3_fx, a);
	...
	b2Body* AddNode(b2Body* parent, const b2Vec2& localAnchor, int32 depth, float32 offset, float32 a)
	{
		...
		shape.SetAsBox(offset, "0.25"_fx * a, b2Vec2((float32)0, -a), 0_fx);
		body->CreateFixture(&shape, density);

		b2Vec2 a1 = b2Vec2(offset, -a);
		b2Vec2 a2 = b2Vec2(-offset, -a);
		b2Body* body1 = AddNode(body, a1, depth + 1, "0.5"_fx * offset, a);
		b2Body* body2 = AddNode(body, a2, depth + 1, "0.5"_fx * offset, a);
