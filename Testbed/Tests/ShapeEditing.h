...
public:
	...
	ShapeEditing()
	{
		...
		shape.SetAsBox(4_fx, 4_fx, b2Vec2(0_fx, 0_fx), 0_fx);
		m_fixture1 = m_body->CreateFixture(&shape, 10_fx);
	...
	void Keyboard(int key)
	{
		...
		case GLFW_KEY_C:
			if (m_fixture2 == NULL)
			{
				b2CircleShape shape;
				shape.m_radius = 3_fx;
				shape.m_p.Set("0.5"_fx, -4_fx);
				m_fixture2 = m_body->CreateFixture(&shape, 10_fx);
