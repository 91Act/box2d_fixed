...
public:
	...
	RayCast()
	{
		...
		{
			m_edge.Set(b2Vec2(-1_fx, 0_fx), b2Vec2(1_fx, 0_fx));
		}
	...
	void Create(int32 index)
	{
		...
		float32 y = RandomFloat(0_fx, 20_fx);
		bd.position.Set(x, y);
	...
	void Step(Settings* settings)
	{
		...
		b2Vec2 d(L * m_angle.cos(), L * m_angle.sin());
		...
		else if (m_mode == e_multiple)
		{
			...
			for (int32 i = 0; i < callback.m_count; ++i)
			{
				...
				g_debugDraw.DrawPoint(p, 5_fx, b2Color("0.4"_fx, "0.9"_fx, "0.4"_fx));
				g_debugDraw.DrawSegment(point1, p, b2Color("0.8"_fx, "0.8"_fx, "0.8"_fx));
				b2Vec2 head = p + "0.5"_fx * n;
				g_debugDraw.DrawSegment(p, head, b2Color("0.9"_fx, "0.9"_fx, "0.4"_fx));
			}
		}
		...
		{
			b2Vec2 vertices[4];
			//vertices[0].Set(-"22.875"_fx, -3_fx);
			//vertices[1].Set("22.875"_fx, -3_fx);
			//vertices[2].Set("22.875"_fx, 3_fx);
			//vertices[3].Set(-"22.875"_fx, 3_fx);

			b2PolygonShape shape;
			//shape.Set(vertices, 4);
			shape.SetAsBox("22.875"_fx, 3_fx);

			b2RayCastInput input;
			input.p1.Set("10.2725"_fx,"1.71372"_fx);
			input.p2.Set("10.2353"_fx,"2.21807"_fx);
			//input.maxFraction = "0.567623"_fx;
			input.maxFraction = "0.56762173"_fx;

			b2Transform xf;
			xf.SetIdentity();
			xf.position.Set(23_fx, 5_fx);
