...
static void sKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS)
	{
		...
		case GLFW_KEY_LEFT:
			...
			{
				b2Vec2 newOrigin(2_fx, 0_fx);
				test->ShiftOrigin(newOrigin);
			}
			else
			{
				g_camera.m_center.x -= "0.5"_fx;
			}
			break;
		...
		case GLFW_KEY_DOWN:
			...
			if (mods == GLFW_MOD_CONTROL)
			{
				b2Vec2 newOrigin(0_fx, 2_fx);
				test->ShiftOrigin(newOrigin);
			}
			else
			{
				g_camera.m_center.y -= "0.5"_fx;
			}
			break;
		...
		case GLFW_KEY_HOME:
			// Reset view
			g_camera.m_zoom = 1_fx;
			g_camera.m_center.Set(0_fx, 20_fx);
			break;

		case GLFW_KEY_Z:
			// Zoom out
			g_camera.m_zoom = b2Min("1.1"_fx * g_camera.m_zoom, 20_fx);
			break;

		case GLFW_KEY_X:
			// Zoom in
			g_camera.m_zoom = b2Max("0.9"_fx * g_camera.m_zoom, "0.02"_fx);
			break;
...
static void sInterface()
{
	...
	if (ui.showMenu)
	{
		...
		ImGui::SliderFloat("##Hertz", &hz, 5.0f, 120.0f, "%.0f hz");
		ImGui::PopItemWidth();
