...
void Test::MouseDown(const b2Vec2& p)
{
	...
	if (callback.m_fixture)
	{
		...
		md.bodyB = body;
		md.target = p;
		md.maxForce = 1000_fx * body->GetMass();
...
void Test::CompleteBombSpawn(const b2Vec2& p)
{
	...
	const float32 multiplier = 30_fx;
...
{
	b2Vec2 p(RandomFloat(-15_fx, 15_fx), 30_fx);
	b2Vec2 v = -5_fx * p;
	LaunchBomb(p, v);
}
...
void Test::LaunchBomb(const b2Vec2& position, const b2Vec2& velocity)
{
	...
	b2Vec2 minV = position - b2Vec2("0.3"_fx,"0.3"_fx);
	b2Vec2 maxV = position + b2Vec2("0.3"_fx,"0.3"_fx);
...
{
	float32 timeStep = settings->hz > 0_fx ? 1_fx / settings->hz : float32(0_fx);
	...
	if (settings->drawProfile)
	{
		...
		if (m_stepCount > 0)
		{
			float32 scale = 1_fx / float32::make(m_stepCount);
	...
	{
		b2Color c;
		c.Set(0_fx, 0_fx, 1_fx);
		g_debugDraw.DrawPoint(m_bombSpawnPoint, 4_fx, c);
	...
	{
		const float32 k_impulseScale = "0.1"_fx;
		const float32 k_axisScale = "0.3"_fx;
		...
		for (int32 i = 0; i < m_pointCount; ++i)
		{
			...
			{
				// Add
				g_debugDraw.DrawPoint(point->position, 10_fx, b2Color("0.3"_fx, "0.95"_fx, "0.3"_fx));
			}
			else if (point->state == b2_persistState)
			{
				// Persist
				g_debugDraw.DrawPoint(point->position, 5_fx, b2Color("0.3"_fx, "0.3"_fx, "0.95"_fx));
			}
			...
			if (settings->drawContactNormals == 1)
			{
				...
				g_debugDraw.DrawSegment(p1, p2, b2Color("0.9"_fx, "0.9"_fx, "0.9"_fx));
			}
			else if (settings->drawContactImpulse == 1)
			{
				b2Vec2 p1 = point->position;
				b2Vec2 p2 = p1 + k_impulseScale * point->normalImpulse * point->normal;
				g_debugDraw.DrawSegment(p1, p2, b2Color("0.9"_fx, "0.9"_fx, "0.3"_fx));
			}

			if (settings->drawFrictionImpulse == 1)
			{
				b2Vec2 tangent = b2Cross(point->normal, 1_fx);
				b2Vec2 p1 = point->position;
