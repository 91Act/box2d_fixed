...
: b2Joint(def)
{
	...
	m_localYAxisA = b2Cross(1_fx, m_localXAxisA);
...
void b2PrismaticJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_enableLimit)
	{
		...
		if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2_fx * b2_linearSlop)
		{
...
void b2PrismaticJoint::SolveVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		if (m_limitState == e_atLowerLimit)
		{
			m_impulse.z = b2Max(m_impulse.z, 0_fx);
		}
		else if (m_limitState == e_atUpperLimit)
		{
			m_impulse.z = b2Min(m_impulse.z, 0_fx);
		}
...
bool b2PrismaticJoint::SolvePositionConstraints(const b2SolverData& data)
{
	...
	if (m_enableLimit)
	{
		...
		else if (translation <= m_lowerTranslation)
		{
			...
			C2 = b2Clamp(translation - m_lowerTranslation + b2_linearSlop, -b2_maxLinearCorrection, 0_fx);
		...
		{
			// Prevent large linear corrections and allow some slop.
			C2 = b2Clamp(translation - m_upperTranslation - b2_linearSlop, 0_fx, b2_maxLinearCorrection);
