...
void b2WeldJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_frequencyHz > 0_fx)
	{
		...
		float32 invM = iA + iB;
		float32 m = invM > 0_fx ? 1_fx / invM : 0_fx;
		...
		// Damping coefficient
		float32 d = 2_fx * m * m_dampingRatio * omega;
		...
		m_gamma = h * (d + h * k);
		m_gamma = m_gamma != 0_fx ? 1_fx / m_gamma : 0_fx;
		m_bias = C * h * k * m_gamma;

		invM += m_gamma;
		m_mass.ez.z = invM != 0_fx ? 1_fx / invM : 0_fx;
	}
	else if (K.ez.z == 0_fx)
	{
		K.GetInverse22(&m_mass);
...
bool b2WeldJoint::SolvePositionConstraints(const b2SolverData& data)
{
	...
	else
	{
		...
		else
		{
			...
			impulse.Set(impulse2.x, impulse2.y, 0_fx);
		}
