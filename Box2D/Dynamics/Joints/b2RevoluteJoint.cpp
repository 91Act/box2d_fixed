...
void b2RevoluteJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_enableLimit && fixedRotation == false)
	{
		...
		if (b2Abs(m_upperAngle - m_lowerAngle) < 2_fx * b2_angularSlop)
		{
...
bool b2RevoluteJoint::SolvePositionConstraints(const b2SolverData& data)
{
	...
	float32 angularError = 0_fx;
	float32 positionError = 0_fx;

	bool fixedRotation = (m_invIA + m_invIB == 0_fx);

	// Solve angular limit constraint.
	if (m_enableLimit && m_limitState != e_inactiveLimit && fixedRotation == false)
	{
		float32 angle = aB - aA - m_referenceAngle;
		float32 limitImpulse = 0_fx;
		...
		else if (m_limitState == e_atLowerLimit)
		{
			...
			C = b2Clamp(C + b2_angularSlop, -b2_maxAngularCorrection, 0_fx);
			limitImpulse = -m_motorMass * C;
		}
		else if (m_limitState == e_atUpperLimit)
		{
			...
			// Prevent large angular corrections and allow some slop.
			C = b2Clamp(C - b2_angularSlop, 0_fx, b2_maxAngularCorrection);
			limitImpulse = -m_motorMass * C;
		}
