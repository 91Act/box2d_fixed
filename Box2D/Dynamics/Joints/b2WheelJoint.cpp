...
void b2WheelJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_frequencyHz > 0_fx)
	{
		...
		if (invMass > 0_fx)
		{
			m_springMass = 1_fx / invMass;

			float32 C = b2Dot(d, m_ax);

			// Frequency
			float32 omega = 2_fx * b2_pi * m_frequencyHz;

			// Damping coefficient
			float32 damp = 2_fx * m_springMass * m_dampingRatio * omega;
			...
			if (m_gamma > 0_fx)
			{
				m_gamma = 1_fx / m_gamma;
			}

			m_bias = C * h * k * m_gamma;

			m_springMass = invMass + m_gamma;
			if (m_springMass > 0_fx)
			{
				m_springMass = 1_fx / m_springMass;
			}
		}
	}
	else
