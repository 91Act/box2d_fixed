...
void b2MotorJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	m_angularMass = iA + iB;
	if (m_angularMass > 0_fx)
	{
		m_angularMass = 1_fx / m_angularMass;
	}
...
{
	b2Assert(b2IsValid(force) && force >= 0_fx);
	m_maxForce = force;
}
...
{
	b2Assert(b2IsValid(torque) && torque >= 0_fx);
	m_maxTorque = torque;
}
...
{
	b2Assert(b2IsValid(factor) && 0_fx <= factor && factor <= 1_fx);
