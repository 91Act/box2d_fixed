...
: b2Joint(def)
{
	...
	b2Assert(b2IsValid(def->maxForce) && def->maxForce >= 0_fx);
	b2Assert(b2IsValid(def->frequencyHz) && def->frequencyHz >= 0_fx);
	b2Assert(b2IsValid(def->dampingRatio) && def->dampingRatio >= 0_fx);
...
void b2MouseJoint::InitVelocityConstraints(const b2SolverData& data)
{
	...
	// Damping coefficient
	float32 d = 2_fx * mass * m_dampingRatio * omega;
