						   ...
						   const b2PolygonShape* polygonB, const b2Transform& xfB)
{
	...
	float32 offset0 = 0_fx, offset2 = 0_fx;
	...
	if (hasVertex0)
	{
		...
		convex1 = b2Cross(edge0, edge1) >= 0_fx;
	...
	if (hasVertex3)
	{
		...
		convex2 = b2Cross(edge1, edge2) > 0_fx;
	...
	if (hasVertex0 && hasVertex3)
	{
		if (convex1 && convex2)
		{
			m_front = offset0 >= 0_fx || offset1 >= 0_fx || offset2 >= 0_fx;
			if (m_front)
			{
				m_normal = m_normal1;
		...
		{
			m_front = offset2 >= 0_fx || (offset0 >= 0_fx && offset1 >= 0_fx);
			if (m_front)
			{
				m_normal = m_normal1;
				m_lowerLimit = m_normal1;
	...
	else if (hasVertex0)
	{
		if (convex1)
		{
			m_front = offset0 >= 0_fx || offset1 >= 0_fx;
			if (m_front)
			{
				m_normal = m_normal1;
				m_lowerLimit = m_normal0;
	...
	else if (hasVertex3)
	{
		if (convex2)
		{
			m_front = offset1 >= 0_fx || offset2 >= 0_fx;
			if (m_front)
			{
				m_normal = m_normal1;
				m_lowerLimit = -m_normal1;
    ...
    const float32 k_relativeTol = "0.98"_fx; // 0.98f;
    const float32 k_absoluteTol = "0.001"_fx; // 0.001f;
...
b2EPAxis b2EPCollider::ComputeEdgeSeparation()
{
	...
	axis.separation = fixed_number::max;
